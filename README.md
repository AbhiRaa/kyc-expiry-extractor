...
> These numbers reflect the 35-document corpus (`eval/generate-corpus.ts`) and
> `eval/results.md` as committed alongside this section — see the repository's commit
> history for the exact revision. If a fresh `npm run eval` disagrees with this README,
> trust the fresh run; regenerating these numbers is exactly what that command is for.
