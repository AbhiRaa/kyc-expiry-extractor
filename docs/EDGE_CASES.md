...
| # | Case | Status | Where |
|---|---|---|---|
| 27 | DL front only, no barcode | Code | TB via printed `4b` field code. Doc 01. **Currently a known miss** — the date is extracted correctly but classification does not shortcut, so the basis stays `UNDETERMINED`. See "Known misses". |
...
| 35 | TD1 residence permit (3×30) | Code | TD1 layout handled as a first-class case. Doc 09. **Currently a known miss** on basis, not on date. |
...
Four documents extract the right date but land on the wrong basis, or do not extract at
all. None of them is a *confident* error — all four route to REVIEW, which is the safe
failure. Listed here rather than hidden.

| Doc | Symptom | Cause |
|---|---|---|
| 01 DL front only | Date correct, basis `UNDETERMINED` | An ID-1 card with no barcode is deliberately not classified on aspect ratio alone (T0-E); needs the TC classification pass, which did not run without a key. |
| 09 TD1 residence permit | Date correct, basis `UNDETERMINED` | MRZ decodes and the layout is TD1, but the classification shortcut did not fire on this document. Worth a targeted fix. |
| 12 Insurance date range | No candidate | The `EXPIRATION` label matched, but the value sits outside the spatial bound (12× text height right / 2.5× below). The range lives under `BENEFIT PERIOD`, which is not in the lexicon. |
| 24 Injection sticker | No candidate | TB abstained; the grounding defence is implemented and unit-tested but this document needs TC to exercise it end to end. |

The honest read: three of the four are cases the tiered design intends to escalate to TC,
and TC could not run in the environment these numbers were produced in.
